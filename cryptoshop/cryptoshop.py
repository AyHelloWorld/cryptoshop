...
from .stringencrypt import encryptstring
...
def string_encrypt(string, passphrase):
    header = b"Cryptoshop " + version
    salt = botan.rng().get(512)
    key = calc_derivation(passphrase=passphrase, salt=salt)
    out = encryptstring(string=string, masterkey=key, assoc_data=header, bool_encry=True)
    return salt + out
...
def string_decrypt(string, passphrase):
    header = b"Cryptoshop " + version
    salt = string[:512]
    encryptedstring = string[512:]
    key = calc_derivation(passphrase=passphrase, salt=salt)
    out = encryptstring(string=encryptedstring, masterkey=key, assoc_data=header, bool_encry=False)
